...
public class CanApprovePullRequestsCommand : ReviewCommand
{
    ...
    public CanApprovePullRequestsCommand(ILoggerFactory loggerFactory, IReviewContextStore contextStore)
        : base(contextStore)
    {
    ...
    {
        return "Allows you to specify if given reviewer can or cannot approve pull requests. If a reviewer that cannot approve pull request is chosen a second one that can will be selected too.";
    }
    ...
    {
        return new CanApprovePullRequestsCommandExecutable(_loggerFactory, this, turnContext, contextStore);
    }

    private class CanApprovePullRequestsCommandExecutable : ReviewCommandExecutable
    {
        public CanApprovePullRequestsCommandExecutable(ILoggerFactory loggerFactory, Command command, ITurnContext turnContext, IReviewContextStore contextStore)
        ...
        protected override IActivity ExecuteReviewAction()
        {
            ...
            if (messageActivity.StripRecipientMention().ContainsAnyOrdinal("can approve pull request"))
            {
                try
                {
                    ...
                    return TurnContext.Activity.CreateReply().AppendText("Wow, that's awesome! Congratulations to your promotion ").AppendMention(reviewerToSet).AppendText("! :)");
                }
            ...
            else
            {
                try
                {
                    ...
                    return TurnContext.Activity.CreateReply().AppendText("Ok. Btw, did you know that reviewers in alltime and status command with star after their name are the ones who cannot approve pull requests?");
                }
                catch (ReviewerNotRegisteredException)
                {
                    return CreateSorryReviewerNotRegisteredReply(reviewerToSet);
