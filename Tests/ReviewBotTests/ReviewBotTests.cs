...
public class ReviewBotTests
{
    ...
    public class FindReviewerCommand
    {
        ...
        [TestCase("FEATURE-1234 @'jira-help' is ready for @'Review'")]
        [TestCase("FEATURE-1234 is ready for @'Review' @'jira-help'")]
        public async Task OnTurnAsync_LookingForReviewerOfYourPullRequestAndOnlyReviewerThatCannotApprovePullRequestIsAvailable_ExpectReviewerWithHighestDebtAssigned(
            string findReviewerMessageText)
        {
            //Arrange
            var reviewBot = MakeReviewBot();
            var registerMessage = MSTeamsTurnContext.CreateUserToBotChannelMessage("@'Review' register @'x x x', @'yyy'");
        ...
        [TestCase("me, @'x x x' @'yyy' are looking for @'Review' of SKYE-1234")]
        [TestCase("Me, @'x x x' @'yyy' are looking for @'Review' of SKYE-1234. It is quite small.")]
        [TestCase("me, @'x x x', @'yyy' are looking for @'Review' of SKYE-1234 @'jira-help'")]
    ...
    public class SetCanApprovePullRequestsCommand
    {
        ...
        public async Task OnTurnAsync_SettingCanApprovePullRequestToFalse_ExpectAcknowledgeMessage()
        {
            ...
            //Assert
            Assert.That(
                makeBusyMessage.Responses.Peek().Text,
                Is.EqualTo("Ok. Btw, did you know that reviewers in alltime and status command with star after their name are the ones who cannot approve pull requests?"));
            Assert.That(allTimeMessage.Responses.Peek().Text, Is.EqualTo("Ordered by review count:\n\n" + "**x x x*** (Available): Reviews: 0, Debt: 0\n\n"));
        }
